...
=======
## Julia{#query_julia}
>>>>>>> de2e9d04b8120505a6fcbddb379b196e981db63d
...
```
<<<<<<< HEAD
The Query package allows users to manipulate data using either LINQ style queries as above, or tidyverse styled queries below.
...
libname conn2 clear;
```
Here we pull the rows that you need and disconnect from the database.

**Query without libname**

While this option means a libname is unnecessary, the syntax is much more convoluted than the above example.
We'd recommend using the libname engine, since it provides a more direct interface to the database driver. 
```sas
proc sql;
connect to odbc as conn
