...
For Statisticians, one of the most useful parts of a web API is the **Query String**.
...
the main url, or **endpoint**, and is separated by a ?. Each new parameter is designated by 
an &. An example could look like:
...
directly into the url string, which should always work. If the package or programming language allows it,
...
but many of them are designed to be familiar. The example below uses a SODA API, who's query string follows
SQL-like syntax. 
